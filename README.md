...
4. As soon as Jenkins finish building image, Jenkins pushes the image to Harbor private repository (Take note to configure the git push job settings from "Git publisher" as "push only if build succeeds", by this way only successfull builds will be pushed), harbor will scann the new image and will "prevent vulnerable images images from running", whe have set "medium" as a level to prevent a image to running;
